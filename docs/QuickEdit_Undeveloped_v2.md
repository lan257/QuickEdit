...
版本：v1.3
日期：2026-09-24
状态：已随 `2.2.0` 发布：v2.0 / v2.1 探索版实现到 Phase 8（详见 §11 状态总表），三轮复验待办 13 条全部实现（§12）；Phase 9 MCP Bridge 尚未开始
依据：
...
| — | 页面背景与正文卡片 | v2.1 | 已完成（4 款渐变预设 + 自选图片、背景可见度、侧栏/正文/批注磨砂、正文卡片可关），原 §13.2 探索项落地 |
| P3 | Hunk 级审批 | v2.3+ | 待评估 |
| P3 | PDF Range Reader | v2.3+ | 待评估 |

## 12. 复验待办（2026-09-22 / 2026-09-24，共 13 条，已随 2.2.0 实现）

| # | 条目 | 实现 | 类型 | 状态 |
|---|---|---|---|---|
| 1 | 设置 · 文本大小上限 | 打开设置回填已保存值，保存后长期生效；该上限同时真正参与大文件判定 | 缺陷 | 已实现（2.2.0） |
| 2 | 脚本运行按钮位置 | 运行图标移到编辑区工具栏 UTF-8 编码标签左侧；不可编辑的文件（如 `.exe`）在信息页提供「在终端运行」 | 增强 | 已实现（2.2.0） |
| 3 | 左侧文件栏宽度 | 右边缘可拖拽调整（200–560px，随窗口收窄），宽度记在本机；双击边缘恢复默认 | 增强 | 已实现（2.2.0） |
| 4 | 暗色下应用名称 | 品牌区文字改用主题语义变量，深色下对比达标 | 缺陷 | 已实现（2.2.0） |
| 5 | 重命名不显示原名 | 重命名输入框预填当前名称并选中主名（不含后缀） | 缺陷 | 已实现（2.2.0） |
| 6 | 上一个 / 下一个文档 | `Alt+↑` / `Alt+↓` 在可见文档间循环切换，不加界面按钮 | 增强 | 已实现（2.2.0） |
| 7 | 重新加载文件夹和文档 | 右键菜单「重新加载」与 `F5`：文档重新读盘（有未保存修改先确认），文件夹/工作区重新扫描子项 | 增强 | 已实现（2.2.0） |
| 8 | Ctrl+F 结果错位 | 查找偏移与编辑器坐标同一空间，忽略大小写改为按位置精确匹配；匹配处在正文高亮，当前匹配单独配色 | 缺陷 | 已实现（2.2.0） |
| 9 | 图片放大缩小交互 | 滚轮以指针为锚点缩放、按住拖动平移（放大后不平移空白、图像不会丢失）、双击切换适应/100% | 增强 | 已实现（2.2.0） |
| 10 | 设置弹窗底部按钮条 | 深色主题下底部一条浅色底，与弹窗其他区域脱节 | 底部条跟随弹窗底色，明暗两主题一致 | 缺陷 | 已实现（2.2.0） |
| 11 | 页面背景 | 无法为界面铺背景 | 4 款内置渐变预设 + 自选本地图片；「背景可见度」滑杆 0–100 只作用于侧栏、批注面板与窗口边缘（磨砂），正文卡片不受影响 | 增强 | 已实现（2.2.0） |
| 12 | 正文卡片 | 正文与窗口底色连成一片，没有质感与区分 | 正文区是浮在背景之上的实心纸面，带描边、圆角与投影，与背景明确区分，可在设置里关闭 | 增强 | 已实现（2.2.0） |
| 13 | 设置弹窗滚不到底 | 弹窗内容变高后底部按钮被裁掉，点不到「保存设置」 | 弹窗改为 flex 列，内容区自身滚动，按钮吸底在任何窗口高度下都可见 | 缺陷 | 已实现（2.2.0） |
...
工作区壁纸、背景可见度、磨砂与正文卡片已实现（§11、§12 第 11–12 条）。仍在探索：

- 间距与描边 token 化，提供紧凑 / 舒适 / 护眼三套预设；
- 主题强调色可自定义 1–2 个色值，语义 token 结构不变；
- 自选背景图片的模糊强度是否要独立于可见度单独可调（当前磨砂固定 16px）。
