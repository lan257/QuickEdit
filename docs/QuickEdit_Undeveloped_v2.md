...
版本：v1.1
日期：2026-09-22
状态：v2.0 / v2.1 探索版已实现到 Phase 8（详见 §11 状态总表）；Phase 9 MCP Bridge 与本轮复验待办（§12）尚未开始
依据：
...
| P1 | 图片 Viewer | v2.1 | 已完成（适应窗口、缩放、旋转、系统打开、Blob 释放） |
...
## 12. v2.1 复验待办（2026-09-22）

| # | 条目 | 现在的表现 | 期望 | 类型 |
|---|---|---|---|---|
| 1 | 设置 · 文本大小上限 | 每次打开设置都要重新填写；填了保存后再次打开仍是空的 | 打开设置时回填已保存的值，保存后长期生效 | 缺陷 |
| 2 | 脚本运行按钮位置 | 运行图标在右上角标题区，位置偏高、离正文太远 | 移到编辑区工具栏 UTF-8 编码标签左侧 | 增强 |
| 3 | 左侧文件栏宽度 | 侧栏宽度固定不可调 | 支持左右拖拽调整宽度，并按工作区记忆 | 增强 |
| 4 | 暗色下应用名称 | 深色主题下侧栏应用名/标题文字看不见 | 暗色下品牌区文字与背景对比达标 | 缺陷 |
| 5 | 重命名不显示原名 | 重命名输入框是空的，不知道当前叫什么 | 预填当前文件名并选中主名（不含后缀） | 缺陷 |
| 6 | 上一个 / 下一个文档 | 无此能力 | 提供快捷键切换上/下一个文档，不加界面按钮 | 增强 |
