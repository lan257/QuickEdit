...
- 统一对象信息页：文件夹、不支持格式、可执行文件、过大文件、加载失败共用一套视图，未支持格式可直接“以文本方式打开”，可运行文件在信息页直接“在终端运行”；
- 图片只读 Viewer：适应窗口、以指针为锚点的滚轮缩放、拖拽平移、双击切换适应/100%、旋转、系统打开，切换文档时释放 Blob；
- 左侧文件栏宽度可拖拽调整并记忆，双击边缘恢复默认；
- `F5` 重新加载当前文档或工作区，`Alt+↑` / `Alt+↓` 在上一个/下一个可见文档间切换；
- 查找匹配在正文中高亮（当前匹配单独配色），忽略大小写按位置精确匹配；编辑保存保留文件原有 CRLF/LF 换行风格；
...
- 页面背景：4 款内置渐变预设或自选本地图片，「背景可见度」滑杆控制侧栏、批注面板与窗口边缘的透明度并加磨砂；正文区是浮在背景之上的实心纸面卡片（描边、圆角、投影），与背景明确区分，可在设置里关闭；
