...
## 12. 复验待办（2026-09-22 / 2026-09-24，共 13 条，已全部实现）
...
| 10 | 设置弹窗底部按钮条 | 深色主题下底部一条浅色底，与弹窗其他区域脱节 | 底部条跟随弹窗底色，明暗两主题一致 | 缺陷 | 已完成，待桌面复验 |
| 11 | 页面背景 | 无法为界面铺背景 | 4 款内置渐变预设 + 自选本地图片；「背景可见度」滑杆 0–100 只作用于侧栏、批注面板与窗口边缘（磨砂），正文卡片不受影响 | 增强 | 已完成，待桌面复验 |
| 12 | 正文卡片 | 正文与窗口底色连成一片，没有质感与区分 | 正文区是浮在背景之上的实心纸面，带描边、圆角与投影，与背景明确区分，可在设置里关闭 | 增强 | 已完成，待桌面复验 |
| 13 | 设置弹窗滚不到底 | 弹窗内容变高后底部按钮被裁掉，点不到「保存设置」 | 弹窗改为 flex 列，内容区自身滚动，按钮吸底在任何窗口高度下都可见 | 缺陷 | 已完成，待桌面复验 |
